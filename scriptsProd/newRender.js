...
var require_newRender = __commonJS(() => {
  var canvas = document.getElementById("gcx");
...
`;
  ...
  async function init() {
    ...
    for (const msg of (await shaderModule.getCompilationInfo()).messages) {
      console[msg.type === "error" ? "error" : "warn"](`WGSL ${msg.type}: ${msg.message} (line ${msg.lineNum})`);
    }
    ctx.configure({
      device,
      format: "bgra8unorm"
    ...
    const clearColour = { r: 0, g: 0, b: 0, a: 1 };
...
export default require_newRender();
